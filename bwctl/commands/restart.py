...
def restart_workload(ctx, workload_name, dry_run, all_nodes):
    """Restart workload"""
    ...
    if not ctx.obj.state.get_current_fabric():
        log_error('Cannot restart {0}. Please select fabric first.'.format(obj_kind))
        sys.exit(1)
    # Check target objects to be used
    if all_nodes:
        if workload_name:
            log_error("Cannot restart. Either {}-NAME or option --all should be used".format(obj_kind.upper()))
            sys.exit(1)
        ...
        if not obj_list:
            log_error("Cannot restart. There are no configured {}s".format(obj_kind))
            sys.exit(1)
    elif workload_name:
        ...
        if not ctx.obj.state.check_workload(ctx.obj.state.get_current_fabric(), obj_name):
            log_error("Cannot restart. {0} {1!r} doesn't exist".format(obj_kind.title(), obj_name))
            sys.exit(1)
        # Check state
        ...
        if ctx.obj.state.check_object_state(obj, ObjectState.DELETING):
            ...
            if not ctx.obj.state.check_object_state(obj, [ObjectState.CONFIGURED, ObjectState.STARTED,
                                                          ObjectState.STOPPED]):
                log_error("Cannot restart. {0} {1!r} should be configured".format(obj_kind.title(), obj_name))
                sys.exit(1)
    ...
    # Restart dry-run
    if dry_run:
        log_warn('{0} {1!r} to be restarted (used with --dry-run)'.format(obj_kind.title(), obj_name))
        return True
    log_info('{0}s to be restarted: {1!r}'.format(obj_kind.title(), obj_list))
    # Restart action
    temp_state = deepcopy(ctx.obj.state)
    res = temp_state.workload_restart(temp_state.get_current_fabric(), obj_list)
...
@restart_cmd.command('processor')
@click.pass_context
@click.argument('processor-name', required=False)
@click.option('--dry-run', required=False, is_flag=True, default=False, show_default=True)
@click.option('--all', 'all_nodes', required=False, is_flag=True, default=False, show_default=True)
def restart_processor(ctx, processor_name, dry_run, all_nodes):
    """Restart processor"""
    ...
    # Restart action
    temp_state = deepcopy(ctx.obj.state)
    res = temp_state.processor_restart(temp_state.get_current_fabric(), obj_list)
    # Set temp state to current and dump it
    ctx.obj.state = deepcopy(temp_state)
    ctx.obj.state.dump()
    if not res[0]:
